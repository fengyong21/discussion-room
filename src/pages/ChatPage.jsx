...
function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    // 实际 API 调用（后续对接真实 API 时启用）
    // try {
    //   const res = await request.post('/chat/send', { message: text, session_id: activeSession })
    //   setMessages(prev => [...prev, { id: Date.now() + 1, role: 'ai', content: res.data.reply, time: new Date() }])
    // } catch (err) {
    //   setMessages(prev => [...prev, { id: Date.now() + 1, role: 'ai', content: '抱歉，请求失败，请稍后重试。', time: new Date() }])
    // } finally {
    //   setIsLoading(false)
    // }
  }
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] -m-8 bg-gray-950 text-gray-100">
      {/* 左侧对话历史 */}
      <div
        className={`${
          sidebarOpen ? 'w-72' : 'w-0'
        } transition-all duration-300 overflow-hidden flex-shrink-0 border-r border-gray-800 bg-gray-900`}
      >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
          ...
          {isLoading && (
            <div className="flex justify-start">
              ...
              <div className="bg-gray-800 text-gray-400 px-4 py-3 rounded-2xl rounded-bl-md">
        ...
        <div className="border-t border-gray-800 bg-gray-900/50 px-4 py-4">
          <div className="max-w-4xl mx-auto">
            <div className="flex items-end gap-3 bg-gray-800 rounded-2xl border border-gray-700 focus-within:border-emerald-500 transition-colors px-4 py-3">
              <textarea
                ref={textareaRef}
                value={inputValue}
